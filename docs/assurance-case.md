...
| Extraction recipes are structurally reviewable | Draft 2020-12 schema, 100-entry backlog checks, filename/index parity, HTTPS/date/uniqueness bibliography tests | v0.11 does not fetch sources, type their authority, map individual claims to sources, or prove a route in any tenant |
...
| Success is defined before the evidence is interpreted and scoring is reproducible | Locked success-profile schema; scorecard SHA-256 binding; validator checks complete criterion coverage, evidence classes, score arithmetic, assessed coverage, must gaps, unknowns, and gate status; one negative test per rule | Approval metadata cannot prove stakeholders chose the right outcomes, and an evaluator can still mis-score a partially met criterion |
...
| Contract dates and timestamps are calendar-plausible | Anchored `YYYY-MM-DD` and RFC 3339 patterns in all seven teardown artifact schemas; pattern tests reject month 13, day 32, hour 24, and trailing text | Patterns do not check day against month (2026-02-31) or leap years |
